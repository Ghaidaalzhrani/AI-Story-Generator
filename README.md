...
This tool asks the user 5 questions, then generates a short, unique story based on their answers using the Gemini API. No two stories are ever the same, even if the same person runs it twice.
...
**Output:** A short fantasy story set in Dammam, following a 19-year-old AI student's journey — freshly generated by the AI model based on those exact choices.
